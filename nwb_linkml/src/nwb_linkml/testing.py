...
def print_h5(file: h5py.File, exclude=("specifications",)) -> str:
    """Display HDF5 file structure in tree-like format.

    https://gist.github.com/EmeEmu/141b503e2d271721ece1fb77b273e1bd
    Parameters
    ----------
    ...
    def view_h5attributes(obj, lines, depth=0,):
        atts = obj.attrs
        deep = "│   " * depth
        ks = atts.keys()
        for i, k in enumerate(ks):
            if i == len(ks) - 1:
                ...
                d = deep + "├──"
            try:
                lines.append(f'{d}🏷️{k} = `{atts[k].decode("utf-8")}`')
            except (UnicodeDecodeError, AttributeError):
                lines.append(f'{d}🏷️{k} = `{atts[k]}`')
        return lines

    def view_h5object(obj, lines, depth=0):
        name = obj.name.split("/")[-1]
        if name in exclude:
            return lines
        deep = "│   " * depth + "├──"
        if type(obj) == h5py._hl.group.Group:
            lines.append(f"{deep}📁{name}")
            lines = view_h5attributes(obj, lines, depth=depth + 1)
            for k in list(obj):
                lines = view_h5object(obj[k], lines, depth=depth + 1)
        else:
            lines.append(f"{deep} 🔢{name} ⚙️{obj.shape}{obj.dtype}")
            lines = view_h5attributes(obj, lines, depth=depth + 1)
        return lines
    ...
    for k in list(file):
        if k in exclude:
            continue
        lines = view_h5object(file[k], lines, depth=0)
    return '\n'.join(lines)