...
class IntracellularStimuliTable(DynamicTable):
    ...
    stimulus_template: Optional[str] = Field(
        None,
...
class IntracellularStimuliTableStimulusTemplate(TimeSeriesReferenceVectorData):
    """
    Column storing the reference to the stimulus template for the recording (rows).
    """

    linkml_meta: ClassVar[LinkML_Meta] = Field(LinkML_Meta(), frozen=True)
    name: Literal["stimulus_template"] = Field("stimulus_template")
    idx_start: int = Field(
        ...,
        description="""Start index into the TimeSeries 'data' and 'timestamp' datasets of the referenced TimeSeries. The first dimension of those arrays is always time.""",
    )
...
IntracellularStimuliTableStimulusTemplate.model_rebuild()
