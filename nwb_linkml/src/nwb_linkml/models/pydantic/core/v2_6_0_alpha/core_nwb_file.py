...
class NWBFile(NWBContainer):
    ...
    analysis: Optional[Dict[str, Union[DynamicTable, NWBContainer]]] = Field(default_factory=dict, description="""Lab-specific and custom scientific analysis of data. There is no defined format for the content of this group - the format is up to the individual user/lab. To facilitate sharing analysis data between labs, the contents here should be stored in standard types (e.g., neurodata_types) and appropriately documented. The file can store lab-specific and custom data analysis without restriction on its form or schema, reducing data formatting restrictions on end users. Such data should be placed in the analysis group. The analysis data should be documented so that it could be shared with other labs.""")
    scratch: Optional[Dict[str, Union[DynamicTable, NWBContainer]]] = Field(default_factory=dict, description="""A place to store one-off analysis results. Data placed here is not intended for sharing. By placing data here, users acknowledge that there is no guarantee that their data meets any standard.""")
...
class NWBFileStimulus(ConfiguredBaseModel):
    ...
    presentation: Optional[Dict[str, TimeSeries]] = Field(default_factory=dict, description="""Stimuli presented during the experiment.""")
    templates: Optional[Dict[str, Union[Images, TimeSeries]]] = Field(default_factory=dict, description="""Template stimuli. Timestamps in templates are based on stimulus design and are relative to the beginning of the stimulus. When templates are used, the stimulus instances must convert presentation times to the experiment`s time reference frame.""")
...
class NWBFileGeneral(ConfiguredBaseModel):
    ...
    devices: Optional[Dict[str, Device]] = Field(default_factory=dict, description="""Description of hardware devices used during experiment, e.g., monitors, ADC boards, microscopes, etc.""")
    ...
    optogenetics: Optional[Dict[str, OptogeneticStimulusSite]] = Field(default_factory=dict, description="""Metadata describing optogenetic stimuluation.""")
    optophysiology: Optional[Dict[str, ImagingPlane]] = Field(default_factory=dict, description="""Metadata related to optophysiology.""")
